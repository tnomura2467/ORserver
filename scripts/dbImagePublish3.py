...
print('Start!!')
Info=[[0 for i in range(8)] for j in range(20)]
...
objnum=0
shelf = cv2.imread('data/ImageFile/soturonnShelf.jpg')
shelfCut=shelf[85:235,135:215]
...
def sqlconnect():
	...
	if not os.path.exists(path+'/ppmfile'):
        ...
        #issue query
        query = ("SELECT FrameCount, ObjectCount, Xmin, Ymin, Width, Height,Depth,TakeBring,ColorImage FROM {} "
             ).format(TBL_NAME)
